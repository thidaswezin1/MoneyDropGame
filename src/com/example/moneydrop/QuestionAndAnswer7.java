...
public class QuestionAndAnswer7 extends Activity implements OnClickListener{
	Button btnSure;
	...
	long totalAmount;
	long presentAmount;
	long dropAmount=0;int randnum2;
	long amount1,amount2=0;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.questionandanswer2);
        Intent intent=getIntent();
        present=intent.getStringExtra(QuestionTypePage7.EXTRA_MESSAGE1);
        presentAmount=Integer.parseInt(present);
        //level=intent.getStringExtra(QuestionTypePage1.EXTRA_MESSAGE2);
        index=intent.getStringExtra(QuestionTypePage7.EXTRA_MESSAGE3);
	...
	public void onClick(View v){ 
		if(amount1>0 && amount2>0){
			new AlertDialog.Builder(this)
			.setMessage("Please put money to only one answer!!!")
			...
			.show();
				...
				if(((amount2>0) &&(amount2<=presentAmount))){
					new AlertDialog.Builder(this)
					...
					if(randnum2!=1){
						totalAmount+=0;
						Intent intent=new Intent(this,ShowCorrectAndAmount7.class);
				...
				if(((amount1>0) &&(amount1<=presentAmount))){
					new AlertDialog.Builder(this)
					.setMessage("Please put money to balance with your amount! Your balance is "+presentAmount)
					.setNeutralButton("Okay", new DialogInterface.OnClickListener() {
						@Override
		...
		else if(((amount1+amount2)>presentAmount) || 
				((amount1+amount2)!=presentAmount) || 
				((amount1+amount2)<presentAmount)){
			new AlertDialog.Builder(this)
			.setMessage("Please put money to balance with your amount! Your balance is "+presentAmount)
			.setNeutralButton("Okay", new DialogInterface.OnClickListener() {
